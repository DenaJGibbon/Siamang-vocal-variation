...
siamangLDdf$site <- recode(siamangLDdf$group,
...
siamangLDdf$group <- factor(siamangLDdf$group, levels=c("Group1", "Group2", "Group3", "Group4", "Group5",
...
SiamangLDGroupActualSD <- sapply( as.data.frame(SiamangLDGroupActual), sd)
Group <- unique(id)
...
# Create a corrplot
corrplot::corrplot (SiamangLDGroupActual,is.corr = F, cl.align.text ='l')

# Create a corrplot
corrplot::corrplot (SiamangLDGroupActualSD,is.corr = F, cl.align.text ='l')
...
# Convert to matrix
matrix.output.ran <- stringdistmatrix(siamangLDdfrandom$LDSequence.ran,
                                  method='lv')
...
CombinedLDDF$Group <- factor(CombinedLDDF$Group, levels=c("Group1", "Group2", "Group3", "Group4", "Group5",
                                                          "Group6", "Group7", "Group8", "Group9","Group10"))


ggboxplot(data =CombinedLDDF,x='Group',y= 'MeanLD',color ='Type',fill='Type',outlier.shape = NA)+
  scale_color_manual(values=c('black','red'))+
  scale_fill_manual(values=c('black','red'))+
  theme(legend.title=element_blank())+xlab('')

LDModelNull <- lme4::lmer(MeanLD ~   (1|Group), data=CombinedLDDF)
LDModel <- lme4::lmer(MeanLD ~ Type + (1|Group), data=CombinedLDDF)
bbmle::AICctab(LDModelNull,LDModel)
summary(LDModel)
hist(residuals(LDModel))
sjPlot::plot_model(LDModel,type='est')
